...
| 70 | [`matches_kickoff_uncertain_round.sql`](./matches_kickoff_uncertain_round.sql) | **"Ikke bekræftet" rammer ikke længere bekræftede kampe (`G135`):** `refresh_kickoff_uncertain()` markerer kun, når det indlærte klokkeslæt bærer over halvdelen af rundens ikke-spillede kampe | Aktiv — tilføjet og **kørt i staging og produktion 17. august 2026**. Idempotent (`create or replace`, samme signatur; efterprøvet med to kørsler i træk). 🔴 **FEJLRETTELSE, aflæst på skærmen af en bruger 17. august 2026:** tre ÆGTE Premier League-kampe fire dage ude stod med "~ tid ikke bekræftet", og to Primera División-kampe med "(ikke bekræftet)". Årsagen er, at `#49` markerede på KLOKKESLÆT alene, og at et pladsholder-klokkeslæt ER turneringens typiske anspilstid — 14:00 UTC er 15.00 i London, altså blackout-slottet. **Det er den indvending, kandidat A blev forkastet på i [aflæsningen 7. august](../docs/reviews/football-data-kickoff-aflaesning-2026-08-07.md); kandidat B undgik A's kalibreringsproblem, men ikke A's kollisionsproblem.** 🟢 **Kan kun FJERNE markeringer, aldrig tilføje** — filen indsnævrer en betingelse, så første kørsel efter migreringen rydder de falske positive og returnerer antallet. ✅ **Sikker at køre når som helst og uafhængigt af et deploy.** Én funktion erstattes; ingen tabel, ingen kolonne, ingen policy, ingen rettighed smalnes. 🟢 **Stadig DISPLAY-ONLY:** hverken `match_lock_at()`, `match_locked()`, en RLS-policy eller `analytics_match_locks` røres — samme sikkerhedsargument som `#49`, og stadig en påstand i testen. 🟢 **Signaturen er UÆNDRET**, så `api/sync-matches.js` ikke skal deployes med: et nyt argument ville have gjort funktionen til en overload og fået PostgREST til at afvise syncens RPC-kald med "could not choose a candidate function". ⚠️ **HORISONTEN PÅ TI DAGE LIGGER I `src/lib/scoring.js` OG IKKE HER, og det er en beslutning:** lå den i SQL, kunne ingen kamp inden for ti dage være markeret, og `ALLE UBEKRAEFTEDE` i `sql/checks/kickoff_coverage.sql` ville blive stum kode — en kontrol, der ikke kan lyse, ligner dækning. Prisen er kendt og valgt: en runde, hvor alle kampe rigtigt spilles samtidig, kan gøre kontrollen rød uden en fejl bag. **Forudsætter #49** (kolonnerne og triggeren). Dækket af `sql/tests/kickoff_uncertain.sql` påstand **10-12** (mindretal markerer ikke, hele runden markerer, præcis halvdelen markerer ikke) mod det RIGTIGE skema på PostgreSQL 16.13, plus fire påstande om horisonten i `src/lib/scoring.test.js`/`time.test.js` — og **fejlen er reproduceret før rettelsen** (en engelsk lørdag med 13.30/16.00×3/18.30, hvor kun 16.00-slottet blev markeret), samt efterprøvet med tre SQL-mutationer og tre JS-mutationer, alle fanget |
