...
3. Klik på knappen **"Connect"** — den sidder i den grønne/mørke bjælke **øverst på siden**.
4. Der åbner et vindue med forbindelsesmuligheder. Find sektionen (eller vælg metoden) **"Session pooler"**. *Vigtigt: vælg netop Session pooler — de andre varianter ("Direct connection" og "Transaction pooler") kan fejle til dette formål.*
5. Kopiér tekstlinjen, der vises. Den ligner:
   `postgresql://postgres.abcdefghijklmnop:[YOUR-PASSWORD]@aws-0-eu-central-1.pooler.supabase.com:5432/postgres`
6. Erstat `[YOUR-PASSWORD]` (inklusive de firkantede parenteser) med databasens adgangskode.
...
Du har nu delt databasens adgangskode i en chat. Det er lav risiko, men god hygiejne at rydde op: gentag nulstillingen fra Trin 1.6 (**Project Settings → Database → Reset database password**). Du behøver ikke gemme den nye kode noget sted — den kan altid nulstilles igen næste gang.
...
|---|---|
| *"password authentication failed"* | Forkert adgangskode i strengen. Nulstil den (Vej 1, Trin 1.6) og indsæt den nye. |
...
| Forbindelsen hænger/timeout | Du har fat i "Direct connection" (kræver IPv6) — gå tilbage og vælg **Session pooler** (Trin 1.4). |
