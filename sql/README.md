...
| # | Fil | Formål | Status |
|---|---|---|---|
| — | `schema.sql` | **Genereret** øjebliksbillede af hele `public`. Kør den i et nyt/staging-projekt i stedet for hele listen | Redigér aldrig i hånden. 🛑 **Filen kan ikke pastes direkte i SQL-editoren.** Tre linjer skal ud først: `\restrict`/`\unrestrict` (linje 5 og sidste linje) er psql-**meta**-kommandoer fra `pg_dump` 17.5+ og giver `42601: syntax error at or near "\"`, og `CREATE SCHEMA public;` giver `42P06: schema "public" already exists` i et projekt, der allerede har skemaet. Begge fejl kommer, før noget er kørt. Opskriften (én `sed`) står i [`../docs/STAGING.md`](../docs/STAGING.md) trin 2 |
