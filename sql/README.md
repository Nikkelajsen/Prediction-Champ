...
| `sql/dev/` | Værktøjer til **staging**, ikke migreringer. To filer: [`simulate_season.sql`](./dev/simulate_season.sql), som spiller en hel sæson igennem for testbrugerne — tips, resultater, stillinger, rating, historier og kåringer — i sin EGEN turnering, som ingen synkronisering kan røre; og [`anonymize_rehearsal.sql`](./dev/anonymize_rehearsal.sql) (`G76`), som installerer to **read-only** opslag, så man kan se, hvad kontolukningen gør, før den køres på et menneske | I hånden i staging-projektets SQL-editor. 🛑 **`simulate_season.sql` aldrig i produktion** — den er selv låst med `sim.arm('JA - DETTE ER STAGING')` og et loft over antallet af brugere. `anonymize_rehearsal.sql` skriver intet og er den ene undtagelse: `rehearsal.leagues()` er skrevet til også at måtte køres mod produktionen, fordi den svarer på, om en liga allerede står uden en levende administrator (`A37`) |
