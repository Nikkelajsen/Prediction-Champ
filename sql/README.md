...
| # | Fil | Formål | Status |
|---|---|---|---|
| — | `schema.sql` | **Genereret** øjebliksbillede af hele `public`. Kør den i et nyt/staging-projekt i stedet for hele listen | Redigér aldrig i hånden. 🛑 **Filen kan ikke pastes direkte i SQL-editoren:** `pg_dump` 17.5+ omkranser dumpet med `\restrict`/`\unrestrict` (linje 5 og sidste linje), som er psql-**meta**-kommandoer — editoren svarer `42601: syntax error at or near "\"`, før noget er kørt. Fjern de to linjer først; opskriften står i [`../docs/STAGING.md`](../docs/STAGING.md) trin 2 |
