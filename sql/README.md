...
| — | [`story_engine_v3_measure.sql`](./story_engine_v3_measure.sql) | **Aflæsning** af v3's dagsmotor: tærsklen (`A35`) og variationen (`A33`) i ét opslag | **Ad hoc-værktøj**, ikke en migrering — tilføjet 21. august 2026. **READ-ONLY:** ingen `insert`/`update`/`delete`/`drop` mod `public`, kun temporære tabeller, som lever i den session, der læser filen — samme form som `sql/checks/` og af samme grund. Modsat `story_engine_v2_measure.sql`, som gen-genererede dagskort for at måle TID, rører denne fil ikke motoren; den måler dens produkt. **De to backlog-rækker er ét opslag og ikke to:** tærsklen 45 ligger i hullet mellem `DAY_RESULT`s loft (8 + 12 + 20 = 40) og de fire tungeste reglers gulv (48+), så andelen af kampdage med ulæst-markering ER andelen, hvor en anden regel end dagens facit vandt — hvilket er `A33`s spørgsmål. Regnestykket står i [`docs/reviews/story-engine-v3-scorerum-2026-08-21.md`](../docs/reviews/story-engine-v3-scorerum-2026-08-21.md). Vis-bar regnes for v3 (`loadDayCard` + 48 timer) og er IKKE analytics-tavlens karrusel-begreb fra v2. **Svaret kommer som en TABEL og ikke som `raise notice`** — samme fælde som v2-målingen. **Dækket af `sql/tests/story_engine_v3_measure.sql` i CI** — en ad hoc-aflæsning får normalt ingen test (`story_engine_v2_measure.sql` har ingen), men denne er undtagelsen: `A32` gjorde produktionsaflæsninger til ejerens arbejde, og en forespørgsel, der fejler i editoren, koster en hel omgang af netop den ressource. Fixturens facit er regnet i hånden, fire mutationer er prøvet mod den og alle fanget, og filen køres TO gange i samme session, så gen-kørbarheden er en påstand |
