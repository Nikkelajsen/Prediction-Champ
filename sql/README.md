...
> [`#72 teams_short_name.sql`](./teams_short_name.sql) og
> [`#73 create_competition.sql`](./create_competition.sql).
>
...
| 47 | [`story_engine_v3.sql`](./story_engine_v3.sql) | Story Engine v3: ét dagskort pr. bruger pr. dag, valgt på `news_value` (grundvægt + størrelse + nærhed, tærskel 45) — `stories.news_value` + `stories_day_slot_uniq`, `generate_daily_stories()` skrevet forfra, `build_round_frames()` og `apply_milestone_stories()` | Aktiv — tilføjet 7. august 2026. Idempotent. **Nummeret er højere end #45–#46, men filen er ÆLDRE end dem** — den blev aldrig indført i denne tabel, og en omnummerering ville flytte de numre, resten af dokumentationen henviser til. Kør den før #45, som kalder `generate_stories_catchup()`. **Kør EFTER #37 og #38, og GEN-KØR bagefter #8** (runde-motoren kalder nu `build_round_frames`) **og `analytics_events.sql`** (tre nye eventnavne). Dækket af `sql/tests/story_engine_daily.sql` i CI. **Skal gen-køres ved hver ændring af dagsmotoren** — og filen er derfor den, der oftest står med et 🔴 i `docs/CHANGELOG.md`. ✅ **Gen-kørt 9. august 2026 i produktion OG staging (`G92`):** `generate_daily_stories()` har fået en tidlig udgang mere — `match_day_complete(p_day)`, placeret efter sidste-dag-udgangen og **før** `delete`. Kravet lå indtil da KUN i matches-triggeren (`rating_trigger_optimization.sql`), som bagstopperen `generate_stories_catchup()` i #38 pr. definition omgår, så én færdigspillet kamp kunne udløse hele dagens kort midt på kampdagen. **Kør INGEN andre filer for at levere den — særligt ikke #38**, hvis rettelse er ren kommentar og hvis gen-kørsel ville rulle hele denne fil tilbage (se advarslen længere nede). Kun funktionen ændres; ingen rækker røres, og et forkert kort, der allerede står, skrives forfra af dagens sidste resultat. ✅ **GEN-KØRT efter `B39` 21. august 2026** — historiernes holdnavne er nu de korte (`coalesce(nullif(short_name, ''), name)`, seks steder), så filen **kræver `#72`** og fejler med `42703` uden den. Ingen rækker røres, og teksten er frossen ved skrivningen — kort navn gælder derfor FREMAD: kort, der allerede står, beholder de lange navne, til dagen skrives forfra. ✅ **GEN-KØRT efter `G144` 21. august 2026** — `_sd_streak` har fået en gren mere (`or naeste_dag = p_day`), så 💤-varianten *"Din stime stoppede ved N"* endelig kan nås: indtil da udløste den kun ved et brud på SAMME kampdag som sidste hit. Kun funktionen ændres; ingen rækker røres, ingen anden fil skal med, og der er **ingen frontend-ændring at merge sammen med** — teksten har ligget i motoren hele tiden. Adfærdsændring ved kørsel: en bruger, hvis stime på 5+ brød i dag, kan nu få stimekortet i stedet for dagens facit. Prisen er ~6 % på dagsmotoren (målt i `sql/tests/story_engine_scale.sql`). Dækket af blok 22 i `sql/tests/story_engine_daily.sql`, som er reglens **første** testdækning overhovedet. 🔶 **Det tog to forsøg, og det er lærepengen værd at gemme:** den første kørsel blev meldt udført, men skema-eksporten bagefter viste, at produktionen stadig bar `th.name` — den gamle krop. En gen-kørsel af en STOR fil giver ingen kvittering, man kan se på, og en editor-fane med filen fra før den seneste merge ser nøjagtig ud som en, der er frisk. **Aflæs derfor kørslen frem for at melde den** — dette svarer `true`, når den nye krop står i basen:
`select pg_get_functiondef('public.generate_daily_stories(date)'::regprocedure) like '%short_name%' as kort_navn_aktivt;` (samme opslag med `public.build_round_frames(text)` for rundekortene). **Samme greb gælder hver senere gen-kørsel** — for `G144` svarer `select pg_get_functiondef('public.generate_daily_stories(date)'::regprocedure) like '%naeste_dag%' as g144_aktiv;` `true`, når 💤-grenen står i basen.  historiernes holdnavne er nu de korte (`coalesce(nullif(short_name, ''), name)`, seks steder), så filen **kræver `#72`** og fejler med `42703` uden den. Ingen rækker røres, og teksten er frossen ved skrivningen — kort navn gælder derfor FREMAD: kort, der allerede står, beholder de lange navne, til dagen skrives forfra. Tidligere gen-kørsler: 9. august 2026 (`G92`), 8. august 2026 (mini-stillingen, `G88`) |
...
| — | [`story_engine_v3_measure.sql`](./story_engine_v3_measure.sql) | **Aflæsning** af v3's dagsmotor: tærsklen (`A35`/`A58`) og variationen (`A33`) i ét opslag | **Ad hoc-værktøj**, ikke en migrering — tilføjet 21. august 2026. **READ-ONLY:** ingen `insert`/`update`/`delete`/`drop` mod `public`, kun temporære tabeller, som lever i den session, der læser filen — samme form som `sql/checks/` og af samme grund. Modsat `story_engine_v2_measure.sql`, som gen-genererede dagskort for at måle TID, rører denne fil ikke motoren; den måler dens produkt. **Backlog-rækkerne er ét opslag og ikke flere:** tærsklen 45 ligger i hullet mellem `DAY_RESULT`s loft (8 + 12 + 20 = 40) og de fire tungeste reglers gulv (48+), så andelen af kampdage med ulæst-markering ER andelen, hvor en anden regel end dagens facit vandt. ✅ **Kørt første gang 21. august 2026:** `A33` blev besvaret og lukket (`DAY_RESULT` er 12,3 % af motorens reelle valgmængde mod v2's 44 % — 35 af de 100 bruger-dage havde slet ingen kandidater), mens `A35` IKKE kunne besvares: fire kampdage af de ti. **Kør den igen**, når kampdagene er der; den svarer da også på `A58`. Aflæsningen: [`docs/reviews/story-engine-v3-aflaesning-2026-08-21.md`](../docs/reviews/story-engine-v3-aflaesning-2026-08-21.md). **Læs altid række 20 og 21 sammen** — samme datasæt, to nævnere, og 21. august gav de to modsatte domme. Regnestykket står i [`docs/reviews/story-engine-v3-scorerum-2026-08-21.md`](../docs/reviews/story-engine-v3-scorerum-2026-08-21.md). Vis-bar regnes for v3 (`loadDayCard` + 48 timer) og er IKKE analytics-tavlens karrusel-begreb fra v2. **Svaret kommer som en TABEL og ikke som `raise notice`** — samme fælde som v2-målingen. **Dækket af `sql/tests/story_engine_v3_measure.sql` i CI** — en ad hoc-aflæsning får normalt ingen test (`story_engine_v2_measure.sql` har ingen), men denne er undtagelsen: `A32` gjorde produktionsaflæsninger til ejerens arbejde, og en forespørgsel, der fejler i editoren, koster en hel omgang af netop den ressource. Fixturens facit er regnet i hånden, fem mutationer er prøvet mod den og alle fanget, og filen køres TRE gange i samme session, så gen-kørbarheden er en påstand |
| 49 | [`matches_kickoff_uncertain.sql`](./matches_kickoff_uncertain.sql) | "Klokkeslættet er ikke bekræftet" som egenskab ved kampen (`G85`): `matches.kickoff_prev_at` + `kickoff_uncertain`, triggeren `matches_remember_previous_kickoff` og `refresh_kickoff_uncertain()` | 🛑 **AFLØST AF `#71` (17. august 2026) — MÅ ALDRIG GEN-KØRES.** Hele maskineriet er fjernet igen: en gen-kørsel genskaber tavst begge kolonner, triggeren og funktionen, uden en eneste fejlmelding. Står i listen nedenfor. Historikken: tilføjet og kørt 8. august 2026; markøren var display-only og flyttede aldrig en lås. Hvorfor den røg igen, står ved `#71` og i `docs/DECISIONS.md`; filen selv er bevaret som den bedste beskrivelse af, hvad der blev forsøgt |
...
| 70 | [`matches_kickoff_uncertain_round.sql`](./matches_kickoff_uncertain_round.sql) | **"Ikke bekræftet" rammer ikke længere bekræftede kampe (`G135`):** `refresh_kickoff_uncertain()` markerer kun, når det indlærte klokkeslæt bærer over halvdelen af rundens ikke-spillede kampe | 🛑 **AFLØST AF `#71` (17. august 2026) — MÅ ALDRIG GEN-KØRES, og fejlen ville være TAVS:** filens `create or replace` genskaber `refresh_kickoff_uncertain()` med en krop, der refererer de droppede kolonner — den oprettes uden fejl, fordi plpgsql først valideres ved kald, og eksploderer så ved første kald. Står i listen nedenfor. Historikken: tilføjet og kørt i staging og produktion 17. august 2026 som rettelse af `G135` (ægte PL-kampe fire dage ude stod som ubekræftede), men samme dags beslutning blev at fjerne hele maskineriet i stedet — dominansreglen indsnævrede gættet uden at genoprette tilliden til det. Se `#71` og `docs/DECISIONS.md` |
| 71 | [`matches_kickoff_uncertain_drop.sql`](./matches_kickoff_uncertain_drop.sql) | **"Ikke bekræftet"-markøren fjernes helt:** dropper triggeren `matches_remember_previous_kickoff`, funktionerne `remember_previous_kickoff()` og `refresh_kickoff_uncertain()` samt kolonnerne `matches.kickoff_uncertain` og `kickoff_prev_at` | Aktiv — tilføjet og **kørt i produktion og staging 17. august 2026** (ejeren, efter mergen af #243 — rækkefølgen er dermed efterlevet). Idempotent (lutter `drop … if exists`; efterprøvet med to kørsler i træk i CI's G124-trin). 🔴 **KØRSELSRÆKKEFØLGEN ER OMVENDT AF `#49`s: kør den FØRST, EFTER at koden uden markøren er merget til `main`.** Indtil mergen læser `job-heartbeat.yml`s kontrol `matches.kickoff_uncertain` hvert 30. minut, og droppes kolonnen først, fejler trinnet med `42703` hver gang. Mellemtilstanden den anden vej — kode merget, kolonner stadig i databasen — er harmløs: ingen kode læser eller skriver dem længere. **Adfærdsændring ved kørsel: "(ikke bekræftet)"/`~` forsvinder fra appen (det skete reelt allerede ved mergen); ingen lås flytter sig** — markøren var display-only fra `#49`s første dag, og verifikation 3 i filen efterprøver det. Beslutningen og prisen står i `docs/DECISIONS.md` (17. august 2026): et klokkeslæt vises herefter uden forbehold, og `sql/checks/kickoff_coverage.sql` er igen bevidst blind for de opdigtede tider i PL/PD/SA. **Gør `#49` og `#70` forbudte at gen-køre** — begge står i listen nedenfor. Kør skema-eksporten bagefter |
| 72 | [`teams_short_name.sql`](./teams_short_name.sql) | **Kort holdnavn til visningen (`B39`):** `teams.short_name`, udfyldt af syncen fra football-data.orgs `shortName` og vist **overalt, hvor et holdnavn står** (`B39`-udvidelsen 21. august 2026 — reglen bor i `src/lib/teams.js`) | Aktiv — tilføjet og **kørt i produktionen 20. august 2026** (ejeren, efter mergen af `#248`). Idempotent (`add column if not exists`). ✅ **Ren tilføjelse og sikker at køre når som helst og uafhængigt af deployet, begge veje:** den gamle kode nævner ikke kolonnen; den nye læser `teams` med `select=*` og SKRIVER først feltet, når læsningen viser, at kolonnen findes (guarden `harShortName` i `api/sync-matches.js` — dét er grunden til, at der ikke behøves en UDRULNING-fil). Ingen policy og ingen rettighed ændres — `teams` har tabel-grants, som dækker nye kolonner. **`teams.name` er urørt og forbliver nøgle:** holdmatchen kører fortsat på det normaliserede fulde navn, netop fordi korte navne kolliderer ("Real Sociedad" ⊂ "Real Sociedad de Fútbol"). NULL betyder "leverandøren har intet kort navn" — Sportmonks-ligaerne (Superligaen, Scotland) får aldrig feltet sat og vises med fulde navne. **Adfærdsændring ved kørsel: ingen alene** — feltet udfyldes af næste `sync-matches`-kørsel pr. turnering (hver 12. time; `shortNamesSet` i kørslens resumé viser antallet), og tip-skærmen viser det derefter. Grundlaget er AFLÆST, ikke læst i dokumentationen: alle 78 hold i PD/PL/SA/BL1 har feltet, aldrig tomt, ingen dubletter ([`docs/reviews/football-data-shortname-aflaesning-2026-08-20.md`](../docs/reviews/football-data-shortname-aflaesning-2026-08-20.md)). Dækket af `planTeamWrites`-testene i `api/sync-matches.test.js` og af `src/lib/teams.test.js`. **`#47` LÆSER kolonnen fra 21. august 2026** og skal derfor køres EFTER denne. Kør skema-eksporten bagefter |
| 73 | [`create_competition.sql`](./create_competition.sql) | **Konkurrence-oprettelsen er ÉN skrivning (`G133`):** `create_competition(navn, liga, mode, …, kampliste)` skriver konkurrencen, opretterens deltagerrække og kampene i samme transaktion — samme mønster som `#57 create_group.sql`, bare med TO vinduer at lukke | Aktiv — tilføjet 21. august 2026 og **kørt i produktion og staging samme dag** (ejeren, før mergen af `G133`-klienten). Idempotent (`create or replace` + revoke/grant). ✅ **Ren tilføjelse og sikker at køre når som helst, også før frontend-mergen** — én funktion og dens grants, ingen policy røres, ingen række ændres; den gamle klient kalder den ikke, og den nye ville allerede virke. 🔴 **Den SKAL være kørt, FØR den nye klient udrulles** — `createCompetition()` (`src/lib/data/competitions.js`) kalder KUN RPC'en, så mod en database uden den svarer oprettelsen 404. **Rækkefølgen er efterlevet:** filen var kørt i begge miljøer, før klienten blev merget. **Adfærdsændring ved kørsel: ingen.** Lukker vinduerne, hvor et fejlet kald to eller tre efterlod en **halv konkurrence** (uden deltager eller uden kampe — synlig i klienten, tom i stillingen, og ingen kontrol leder efter den); `G130` viste, hvor kort lunten er. De tre insert-policies står urørte med vilje, så den gamle klient virker, til deployet er ude — en senere smalning ville være sin egen `#58`-agtige opgave og er bevidst ikke bestilt. Dækket af `sql/tests/create_competition.sql` i CI, kørt mod det RIGTIGE skema på PostgreSQL 16.13: begge vinduer fremkaldes med en spærre, den negative kontrol viser den gamle vejs halve konkurrence ved siden af, og et opdigtet kamp-id fælder hele kaldet på fremmednøglen |
| 74 | [`championship_selectors.sql`](./championship_selectors.sql) | **Championship-fanens to vælgere svares af databasen (`G146`):** de to `distinct`-views `championship_rounds(scope, round_key)` og `championship_months(scope, month)`, begge `security_invoker`. Afløser to klientside-`Set` i `loadRoundsAvailable()`/`loadMonthsAvailable()`, som hentede henholdsvis én række pr. spillet kamp og én pr. bruger pr. måned for at svare med et par snese værdier. `scope` har ordret `round_standings`' egen betydning. **Ren tilføjelse — men skal køres FØR frontend-mergen**, ellers svarer viewene `404` (samme rækkefølge-regel som `#62`). Ækvivalensen mod `monthly_standings` og mod den gamle klientvej måles i [`tests/championship_selectors.sql`](./tests/championship_selectors.sql) | Aktiv — tilføjet og **kørt i produktion 21. august 2026, FØR frontend-mergen** som rækkefølge-reglen i filens hoved kræver. Idempotent |
...
- **`security_hardening.sql`** (#27) genskaber `matches_insert_admin` og
  ...
  14. august 2026.)*
- **`matches_kickoff_uncertain.sql`** (#49) genskaber hele det maskineri, som
  `matches_kickoff_uncertain_drop.sql` (#71) har fjernet: kolonnerne
  `kickoff_uncertain` og `kickoff_prev_at`, triggeren
  `matches_remember_previous_kickoff` og `refresh_kickoff_uncertain()` — alt
  sammen tavst, fordi filen er bygget af `if not exists`/`create or replace`.
  Ingen kode kalder eller læser objekterne længere, så symptomet er kun, at et
  fjernet gæt begynder at blive beregnet igen i det stille. **Rettelsen er at
  køre `#71` igen.** *(Tilføjet til listen 17. august 2026.)*
- **`matches_kickoff_uncertain_round.sql`** (#70) er værre end #49, fordi dens
  fejl er FORSINKET: `create or replace` genskaber `refresh_kickoff_uncertain()`
  med en krop, der refererer kolonner, `#71` har droppet — funktionen oprettes
  uden fejl (plpgsql valideres først ved kald) og eksploderer så ved første
  kald. Der ER ingen kalder i koden længere, så den ligger bare og venter på
  den første, der prøver. **Rettelsen er at køre `#71` igen.** *(Tilføjet til
  listen 17. august 2026.)*
