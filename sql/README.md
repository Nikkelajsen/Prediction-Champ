...
| 64 | [`tournament_scotland_promote.sql`](./tournament_scotland_promote.sql) | **Scotland Premiership bliver officiel (`A55`):** `is_visible` + `is_official` = true på `501`, efterfulgt af et `recompute_ratings()` | **Data, ikke skema** — indgår derfor ikke i `schema.sql`. Idempotent. ⚠️ **Kør den MELLEM to spillerunder**, ikke midt i en: sætningen er lige gyldig når som helst, men forfremmelsen ændrer kåringen af en runde, folk allerede har tippet, mens de tipper den. Samme betingelse som #24 skrev om netop denne turnering. **Virkningen er bagudrettet og med vilje:** stillingerne er views og tæller skotske tips med i alle historiske runder ved næste opslag, og `recompute_ratings()` er en fuld genopbygning fra runde nul — derfor kalder filen den selv, så skiftet sker på ét kendt tidspunkt frem for ved næste tilfældige målscoring. Forudsætter #20 og #21 |
| 65 | [`job_health_rate.sql`](./job_health_rate.sql) | **Driftskortet kan se en fejlRATE (`G115`):** `admin_job_health()` svarer også `recent_runs` og `recent_failures` over de sidste 24 timer | Aktiv — tilføjet 14. august 2026. Idempotent (efterprøvet med to kørsler i træk). ✅ **Sikker at køre når som helst og uafhængigt af deployet, begge veje.** Den gamle klient læser svaret felt for felt og ignorerer de to nye nøgler; den nye behandler dem som UMÅLTE, hvis filen ikke er kørt endnu, og opfører sig da nøjagtig som i dag — en umålt rate må ikke kunne forveksles med nul (dækket af en test i `src/lib/ops.test.js`). Ingen tabel, ingen policy, ingen rettighed smalnes. **Adfærdsændring ved kørsel: ja, og den er hele formålet** — et job, der fejler mindst 10 % af mindst fem kørsler i døgnet, vises herefter som `ustabil`, også når den seneste kørsel lykkedes. 🔴 **Hvorfor rækken findes:** `consecutive_failures` nulstilles af enhver succes, så `sync-live` kunne fejle to ud af tre minutter i en time og stå på **OK, 0 fejl i træk** hele vejen igennem — hvilket det gjorde under `G109` 14. august 2026. Det er `B8`s og `G44`s fejlklasse en tredje gang: en sund måling, der skjuler en syg. ⚠️ **Gør `#18 job_runs.sql` umulig at gen-køre i sin helhed** — se listen nedenfor. `drop function` før `create`, fordi returtypen ændrer sig; `grant execute` gentages derfor, da rettighederne følger funktionen i graven. Dækket af `sql/tests/job_health_rate.sql` i CI, kørt mod det RIGTIGE skema på PostgreSQL 16.13 og efterprøvet med tre mutationer — alle tre fanget |
