...
> Kun `on functions` står tilbage — med vilje, for de to funktioner, der kaldes
> FØR login: `username_available()` (oprettelsen) og siden 11. august 2026
> `invite_preview()` (`I7` — invitationens etiket).
>
> ⚠️ **Vær præcis om, hvad "on functions" betyder** (fundet under `I7`): reglen
> er `grant all on functions to anon`, altså **hver eneste** funktion i `public`,
> ikke kun de to. Et `revoke execute … from public` i en migrering fjerner
> PUBLIC's adgang, men ikke `anon`s egen. Det, der holder en fremmed ude af fx
> `invite_lookup()` og `accept_invite()`, er derfor deres egen
> `if auth.uid() is null then raise 'forbidden'` — **den linje er bærende og
> ikke en dobbeltsikring.** `sql/tests/invite_preview.sql` måler den som ADFÆRD
> af netop den grund; en test på grants ville måle noget andet, end den tror.
> Selve oprydningen (en `anon_grants_functions.sql`, der vender defaulten og
> giver de to eksplicit) står i backloggens indbakke.
>
...
| Mappe | Hvad | Køres |
|---|---|---|
| `sql/tests/` | SQL-testene, som CI's `sql`-job kører mod en rigtig PostgreSQL. **Én af dem er ikke en test af én migrering, men af en EGENSKAB ved hele skemaet:** [`write_surface.sql`](./tests/write_surface.sql) vogter, hvad en indlogget bruger kan skrive — fortegnelsen over skrive-policies, at ingen skrivbar view mangler `security_invoker`, `profiles`' kolonneliste og tyve fjendtlige skrivninger. Den er revisionen efter `B29` skrevet som en påstand, så en ny `grant` eller policy ikke kan udvide fladen ubemærket | Af CI. Bruger `\ir`/`\set` med vilje og kan derfor **ikke** pastes i SQL-editoren |
