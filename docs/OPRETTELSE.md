...
| Værtsnavne på widgeten | `prediction-champ.vercel.app` **+ `app.leagly.app`** | `prediction-champ.vercel.app`: **12. august 2026 — BEVIST.** Widgeten viser "Succes!" på produktionens login-skærm, og `window.turnstile.getResponse()` returnerede en token. Værtsnavnet accepteres altså, og ikke bare "widgeten tegnes". `app.leagly.app`: ✅ **21. august 2026 — BEVIST.** Tilføjet 13. august (`I10` trin 5, meldt af ejeren) og afgjort 21. august af `I10`s trin 8: et rigtigt login lykkedes på den nye adresse ([`DOMAENE.md`](./DOMAENE.md) bevis 3). Et login kan ikke lykkes, hvis Cloudflare afviser værtsnavnet — derfor måler beviset indstillingen og ikke widgetens tegning. **Det gamle værtsnavn bliver stående**, indtil redirectet har stået i noget tid: et skifte frem for en udvidelse er præcis den fejl, hvis symptom er, at ingen kan logge ind |
