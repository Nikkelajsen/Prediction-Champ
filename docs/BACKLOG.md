...
| A17 | **Skal ratingen kun tælle officielle turneringer — og skal `ratings.scope` tages i brug?** | A2 gav stillingerne to niveauer og lod `leagues.is_official` afgøre, hvad der fodrer Championship. **Ratingen fik ikke samme behandling:** `recompute_ratings()` (`sql/rating_core.sql`, linje 153–157) joiner `predictions → matches` helt uden liga-filter, så skotske resultater flytter "Prediction Champ Rating", men ingen titler. Skævheden er mindre, end den lyder — rundescoren er `pts / n` (gennemsnit), så bredde giver ikke gratis rating — men spillerne sammenlignes stadig på **delvist forskellige kampsæt** i samme runde, og A2's egen begrundelse ("de eneste konkurrencer, ingen selv har valgt, blev afgjort af valg truffet andre steder") gælder ordret. Tabellerne har allerede en `scope`-kolonne, hårdkodet til `'ALL'`; K2 noterede per-turnering-rating som ubesluttet, men uden en række. Tre mulige svar: lad ratingen være global (og skriv hvorfor), filtrér den på `is_official`, eller tag `scope` i brug og giv hver turnering sin egen rating. | Når Skotland har kørt et par runder, så forskellen kan aflæses i `rating_history` frem for gættes. |
