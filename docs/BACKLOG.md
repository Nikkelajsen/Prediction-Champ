...
| A20 | **Skal en konkurrence efterfyldes med kampe, der først udgives senere?** | En konkurrence er i dag en **frossen kampliste**: alle fem modes materialiserer kampene i `competition_matches` ved oprettelsen (`src/lib/data/competitions.js`), og der findes ingen backfill — hverken trigger, cron eller ops-funktion. Det er en bevidst beslutning ([`DECISIONS.md`](./DECISIONS.md), juli 2026: *"creation-time filter … ikke live-auto-tilknytning"*), og prisen var dengang formuleret som "en playoff-konkurrence oprettes, når kampene er skemalagt". **De syv turneringer har ændret prisen:** Champions League har endnu ingen kampe (`B8`), så en "hel sæson · Champions League" oprettet i dag bliver tom — og forbliver tom, også når lodtrækningen er foretaget. Samme mønster rammer Superliga-slutspillet og enhver knockout-fase. Spørgsmålet er ikke, om beslutningen var rigtig, men om dens forudsætning (én turnering, hvis kampprogram er kendt fra sæsonstart) stadig holder. **Gater `I14`:** en omdøbning, der kalder mode'en "Sæson", gør udeladelsen mere synlig, ikke mindre. | Sammen med `B8`, når det vides, om CL's tomme kampliste er en uudgivet sæson eller en forkert `api_season_id`. |
