...
| # | Hvad | Hvorfor her |
|---|---|---|
| `B12` | Kør §5F-forespørgslen i [`features/analytics-v1.md`](./features/analytics-v1.md) — **den mod `competitions`** | **Forsøgt to gange 5. august 2026.** Første gang kunne forespørgslen ikke køre (`42803`); efter rettelsen svarede den med tre hændelser i alt, alle `random`, fordi `analytics_events` først findes fra 30. juli og "før mærkatet" derfor kun var to døgn. Kilden er byttet til `competitions`, som har hele historikken som rigtige rækker. **Svaret mangler stadig.** Samme opslag svarer på `I15`s spørgsmål om Ugens kupon-kortet. |
