...
| B14 | **Feedback-knap: en vej fra brugeren til udvikleren** | Der findes i dag ingen. En fejl, en forvirring eller et ønske dør, hvor det opstår, og det eneste, der når frem, er det, en bruger husker at sige i telefonen. Det er samme hul som `G42` set fra den anden side: dér mangler maskinens spor efter et crash, her mangler menneskets. **Det ubesluttede er, hvor beskeden lander.** En `feedback`-tabel med RLS (insert for `authenticated`, læsning kun for admin) plus et panel i Admin passer til arkitekturen — ingen ny afhængighed, ingen tredjepart, samme mønster som `analytics_events`. En `mailto:`-genvej er billigere, men åbner brugerens egen mailklient (på iOS ofte slet ingen) og efterlader ingen liste at arbejde ud fra. Beskeden bør bære commit-SHA og skærmnavn automatisk — uden dem er halvdelen af meldingerne umulige at følge op på, og SHA'en står allerede nederst i "Sådan virker det". Hvor knappen hører hjemme, er en selvstændig lille beslutning: Profil er det oplagte, men det er også det sted, ingen er, når noget går galt. | Lille–mellem |
