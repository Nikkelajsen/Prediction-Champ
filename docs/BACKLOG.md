...
*Ryddet 3. august 2026 (tredje runde): den ene linje om kontakt-mailen fik intet eget
ID — den er **foldet ind i `I10`**, som allerede ejer spørgsmålet, og som samme dag
fik sin kravafklaring skrevet ind. Pladsholderen i `site/om.html` er ét af de fem
steder, rækken nu lister, og ville som egen række have været en påmindelse om en
delmængde af noget, der stod lige ved siden af.*
...
| `I10` | Domæne og professionel e-mail | Forudsætning for troværdighed i invitationer og kontakt. Kravafklaret 3. august 2026 (se ID-tabellen): det dyre valg er ikke navnet, men om **appen** skifter origin — det koster hver bruger et log ud, en ny push-tilmelding og en geninstalleret PWA, og prisen vokser med brugerantallet. Skal appen flyttes, er argumentet for at gøre det **nu** stærkere end argumentet for at vente. |
...
| I10 | **Domæne og professionel e-mail** | Forudsætning for troværdighed udadtil (hjemmeside, invitationer, kontakt) — hænger sammen med `I8`. **Kravafklaring 3. august 2026.** Selve købet ligger uden for repoet (registrar + DNS + MitID ved `.dk`), men **beslutningen er ikke navnet — det er, om APPEN skifter origin.** Tre modeller: kun sitet på domænet (appen bliver på `prediction-champ.vercel.app` — ingen brud, kan gøres i dag), sitet på roden + appen på `app.<domæne>`, eller appen på roden. De to sidste koster det samme **engangsbrud for hver bruger**, og det er rækkens egentlige indhold: `refresh_token` og alle ni `localStorage`-flag er origin-bundne (alle logges ud, onboarding-flag nulstilles), service worker og push-abonnement er origin-bundne (alle skal tilmelde sig igen, mens de gamle rækker i `push_subscriptions` bliver ved med at virke mod det *gamle* origin), og en installeret PWA på hjemmeskærmen peger fortsat på det gamle origin, til den geninstalleres. **En redirect redder ingen af delene** — de følger origin, ikke HTTP-svaret. Intet i Supabase røres, og appen har ingen hårdkodet origin at rette (`manifest.json` er relativ, notifikationernes deeplink er `/?…`, invitationslinks bygges af `window.location.origin` i `GroupScreen.jsx:34`/`BoardScreen.jsx:122`) — prisen er alene brugernes lokale tilstand, og den vokser med brugerantallet. **Selve arbejdet er fem steder:** pladsholderen i `site/om.html:88`; `[NAVN]` + `[KONTAKT-E-MAIL]` i `src/lib/legal.js:20-21` (§24 forbyder udtrykkeligt offentliggørelse med pladsholdere i — de er dataansvarlig og kontaktpunkt); `VAPID_SUBJECT` i Vercel, som i dag falder tilbage på `mailto:notifications@prediction-champ.invalid` (`api/send-notifications.js:456`); **Site URL i Supabase → Auth**, fordi `recover()` ikke sender nogen `redirectTo` (`src/lib/supabase.js:132`) og nulstillingslinket derfor bygges udelukkende af den indstilling; og — kun ved origin-skift — de ~18 CTA-links i `site/*.html` plus `README.md:9`. `vercel.json`s CSP er `font-src 'self'` med selv-hostede fonte i begge mapper og skal **ikke** røres. **E-mail er tre uafhængige niveauer**, ikke ét: videresendelse (dækker kontaktadressen), rigtig postkasse (nødvendig for at svare *fra* adressen), og afsenderdomæne til systemmails — det sidste er det eneste, der kræver SPF/DKIM/DMARC, og det eneste, der løser, at bekræftelses- og nulstillingsmails i dag sendes af Supabases delte SMTP med dens meget lave timerate. | Afhænger af I8 — kravafklaring skrevet, beslutning (navn + model) mangler |
