...
| # | Hvad | Note |
|---|---|---|
| `B39` | Kort holdnavn til visningen — **næste skridt er en aflæsning, ikke kode** | Sender football-data.org et kort holdnavn, findes det for HVERT hold, og er det faktisk pænere? **Bestillingen ligger klar** (20. august 2026): [`football-data-shortname-aflaesning-2026-08-20.md`](./reviews/football-data-shortname-aflaesning-2026-08-20.md) — ét PowerShell-opslag med `FOOTBALLDATA_TOKEN`, svaret skrives ind i samme dokument. Egress til leverandøren er blokeret fra udviklingsmiljøet (efterprøvet igen 20. august: proxy-afvist), så svaret kan pr. konstruktion ikke hentes i repoet — derfor Tier 1 og ikke Tier 3. Selve bygningen er lille og beskrevet under [Ubygget](#ubygget). |
...
### 20. august 2026 (treogtredivte kørsel) — Tier 1 kørt: `B39`s aflæsning er bestilt, ikke lukket

**Listen er uændret 35, og rækken bliver stående.** Tier 1's ene række kan pr.
konstruktion ikke lukkes fra repoet, og det blev efterprøvet frem for antaget:
udviklingsmiljøets proxy afviser CONNECT mod `api.football-data.org` med 403,
og `FOOTBALLDATA_TOKEN` findes ikke i miljøet. `A32` står ved magt — aflæsningen
er ejerens.

**Det, tieret ejer, er bestillingen, og den lå ikke klar.** Rækken henviste til
kickoff-aflæsningens PowerShell-opslag, men det udtrækker `utcDate`/`status` og
kan ikke svare på ét af `B39`s tre spørgsmål. Bestillingen er nu skrevet:
[`docs/reviews/football-data-shortname-aflaesning-2026-08-20.md`](./reviews/football-data-shortname-aflaesning-2026-08-20.md)
— ét opslag over de fire aflæselige turneringer (CL tåler at fejle; det er
`B28`s udløser), som svarer på de tre spørgsmål plus en fjerde billig
(dublet-`shortName` inden for samme turnering), en tabel over, hvad hvert
udfald afgør, og et Svar-afsnit, der venter på udskriften. Opslaget rammer med
vilje samme endpoint, syncen henter, og ikke `/teams` — kolonnen skal udfyldes
af det svar, syncen faktisk får.
