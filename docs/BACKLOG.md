...
| `I9` | SEO | ⚠️ **Ikke længere gated 14. august 2026 — siden er publiceret**, så indekseringen og Search Console kan køres. **Skrumpet 13. august 2026:** metadataen er leveret med hjemmesidens andet udkast (canonical, favicon, apple-touch-icon, theme-color, `og:`/`twitter:`-tags, `robots.txt`, `sitemap.xml`). Tilbage er det, der kræver en publiceret side: indeksering, Search Console og en aflæsning af, om `robots.txt`/`sitemap.xml` faktisk serveres fra roden (`A50`). |
