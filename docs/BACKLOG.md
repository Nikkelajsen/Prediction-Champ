...
- B31 (lukket 13. august) har ingen arkivpost i CHANGELOG/DECISIONS — genskab hvad rækken var, eller notér hullet; "intet går tabt"-løftet har dét ene hul.
- Sitet kalder turneringen "Skotske Premiership", basen "Scotland Premiership" — mangler samme afgørelse, som La Liga fik med A49.
- Scotland Premiership er is_official=false: tips dér giver hverken rating eller championship-point, mens sitet sælger den som den anden live-turnering — afgør forfremmelse eller et forbehold i copy.
- Beta-mærkatet findes kun på hjemmesiden; appen siger intet om beta — afgør om de to skal følges ad.
- App-origin er indekserbar og konkurrerer med leagly.app om brand-søgningen: overvej robots.txt med Disallow på appen (eller canonical), plus 404.html, og:locale/og:image:alt, cache-header på /css/ og footer-år på sitet.
- manifest.json kunne få id/scope/lang/screenshots, så Chrome viser den rige installationsprompt.
...
### 14. august 2026 (attende kørsel) — Fuld konsistensgennemgang: rettelser i næsten alle dokumenter, ni rå linjer i indbakken

**Ingen rækker leveret eller lukket — kørslen var en gennemgang, ikke en
leverance.** App, hjemmeside og dokumentation blev holdt op mod hinanden
(seks parallelle gennemlæsninger, ~60 uoverensstemmelser rettet — se
`CHANGELOG.md` 14. august). I denne fil skete tre ting:

- **Tre stale celler ført ajour:** `I10`-rækkerne sagde stadig, at bevis 6 og 7
  udestod (bestået samme dag), og `I9`-opslagsrækken hang på `I8`, der er
  leveret, og pegede på `B30`/`B31`, der er slettet.
- **`A46`-rækken i "Åbne beslutninger" er slettet** efter filens egen regel —
  beslutningen ER truffet (`DECISIONS.md` 13. august 2026); det åbne (aflæsningen
  af de ni værtsnavne) bor i Tier 6-rækken, som står urørt.
- **Indbakken har fået ni rå linjer** fra gennemgangen: et arkivhul (B31), to
  Skotlands-spørgsmål (navnet på sitet; `is_official=false` mod sitets
  live-flagskib), Beta-mærkatets asymmetri, app-originens indekserbarhed +
  site-småting, manifestets installationsfelter, story-vagtens rækkevidde, en
  vagt-idé mod antalsdrift, og skema-eksportens manglende "GENERERET"-hoved.

**Tallet i "Alle 30 åbne punkter" er uændret** — der kom intet ID til og gik
intet fra; indbakken er bevidst ikke ryddet i samme kørsel, som fyldte den.
