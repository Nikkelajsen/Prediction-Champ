...
|---|---|---|---|
| K2 | **Per-turnering-titler i karriereprofilen** | **Afgjort 31. juli 2026, mangler at blive bygget.** Championship kårer nu på to niveauer (`sql/tournament_scope.sql`), og per-turnering-kåringer skal tælle som titler, men **vises adskilt** fra de samlede — så "Månedens Prediction Champ ×5" betyder det samme før og efter turnering #3. Udestår: `titles.by_tournament` i `career_profile.sql` + en egen gruppe med `InfoDot` i `ProfileScreen.jsx`. Spec: [`features/karriereprofil-v1.md`](./features/karriereprofil-v1.md) §8. `ratings.scope` (per-turnering-**rating**) er stadig ubesluttet og bevidst ikke en del af dette. | Lille |
