...
**Kørt tom FEMTE gang 7. august 2026 (nat) — og denne gang tog det tre rækker
med sig, som ikke var der om morgenen.** `G76` er besvaret af virkeligheden:
aflæsningen af `auth.users.deleted_at` viste, at hele kontolukningen ALLEREDE er
kørt i produktion 5. august, 69 ms fra RPC til soft-sletning, uden den halve
tilstand rækken frygtede. `A37` blev fundet, aflæst og lukket samme dag. `A36`
faldt med den. **Tilbage står `G81` alene** — det eneste punkt, der stadig
kræver en adgang, ingen maskine i repoet har.

**Dagen er tieret i miniature, og læren er ikke den, tieret blev skrevet på.**
Tre af de fire rækker blev lukket UDEN den produktionsadgang, de stod her for:
`G76` af et opslag, `A37` af en prøvekørsel mod `sql/schema.sql`, `A36` af en
beslutning. Kun `G81` var, hvad hele tieret påstod at være. **"Svaret ligger uden
for repoet" var rigtigt om adgangen og forkert om arbejdet** — det meste af det
lå i repoet hele tiden.
