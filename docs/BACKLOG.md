...
- `grant all` til `authenticated` står på 29 tabeller ud over `profiles` — en policy kan ikke afgrænse kolonner, så hvilke andre kolonner er kun beskyttet af, at ingen har prøvet?
...
### 10. august 2026 (anden kørsel) — fem punkter lukket, ét bygget, og ét hul fundet undervejs

**Listen er 32 → 27.** Ejeren har afgjort fire ventende spørgsmål, og to rækker
er bygget. Ingen af de fem lukkede ventede på kode; de ventede på et svar.

**Afgjort af produktejeren:** `A5` (emojis bliver i historie-kortene), `A27`
(`competitions.rules` bliver stående, i tilfælde af at pointvariation bliver
aktuelt), `A32` (aflæsninger i produktion er ejerens arbejde — der bygges ingen
vej udenom) og `A36` (kontolukningens ligaregel står, som den er). Begrundelserne
står i [`DECISIONS.md`](./DECISIONS.md).

**`A26` er kørt.** `ambiguousTeams` filtreres nu mod `GODKENDTE_HOLDPAR` i
`api/sync-matches.js`, så feltet igen kun melder det, ingen har set på —
`Dundee`/`Dundee United` er listens eneste række. `ambiguousKnown` er tallet, der
viser, at filteret stadig bider; uden det ville en forkert linje i listen kunne
sluge et ægte fund i tavshed.

**`B29` er bygget — og fandt noget, rækken ikke handlede om.** Brugernavnet kan
nu skiftes fra ens egen karriereprofil. Spørgsmålet "hvilken rettighed skriver
skærmen med?" afslørede, at `grant all on profiles to authenticated` + policyen
`update own profile` lod **enhver bruger sætte sin egen `is_admin`** — altså
admin-vagten i fem funktioner. Efterprøvet mod `sql/schema.sql`: `UPDATE 1`.
🔴 **`sql/username_change.sql` (#51) skal køres i Supabase**, og den skal køres
FØR frontend-mergen: uden den er navneskiftet en skærm bygget oven på en for bred
rettighed. Der er intet i produktionen, der går i stykker af kørslen — den
smalner en rettighed, ingen kode bruger bredt.

**Indbakken bærer én linje**, og det er den generelle udgave af samme fund:
`grant all` står på 29 tabeller ud over `profiles`, og en policy kan ikke
afgrænse kolonner.
