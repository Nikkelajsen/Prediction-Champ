...
gennemgangen af hjemmesidens ventende opdateringsudkast blev til `A48`–`A50`,
`B30`–`B33`, `G103` og `I22` — ingen leveret endnu. Se
...
Alle 42 åbne punkter i den rækkefølge, de bør tages — ikke efter ID og ikke efter
...
| `A48` | Hvad skal opfyldes, før "Beta" fjernes fra hjemmesidens header/forside? | Ejerens beslutning — intet Beta-mærkat er fundet i det ventende udkast, som spørgsmålet stammer fra. |
...
| `B31` | Ryd/skrump `I17` og `I9`s rækker i denne fil | Når den ventende site-opdatering (`B30`) merges. |
...
| `G103` | Sitets story-eksempler er nu Story Engine v3's ægte formuleringer — ingen vagt holder dem i trit med `sql/story_engine_v3.sql` | Samme udløser som `B31`: udkastet merges, og eksemplerne bliver til rigtig, koblet kode. |
...
| # | Hvad | Bemærkning |
|---|---|---|
| `I8` | Professionel hjemmeside | **Første udkast leveret i `site/` (3. august 2026, [`features/hjemmeside-v1.md`](./features/hjemmeside-v1.md)) — ikke publiceret.** **Et opdateringsudkast er til gennemgang 13. august 2026** (fodbold-eksplicit forside, ny turnerings-sektion, SEO-metadata) — **endnu ikke merget, se `B30`.** Tilbage står i øvrigt ejer-godkendelse af copy, domæne og publicering. Gater fortsat `I9` og `I10`, men de har nu noget konkret at hænge på. |
| `B30` | Ret sælgesætningen fodbold-eksplicit og synk den i alle fem filer | Blokerer det ventende udkast af `I8`: `saelgesaetning.test.js` (`G97`) går rødt, fordi udkastets `site/index.html`-description ikke længere begynder med `index.html`s sælgesætning. Se `B31` for hvad udkastet leverer, når det er rettet og merget. |
...
| `I9` | SEO | Der er ingen side at optimere, før `I8` findes. **Dele af den (canonical, favicon, apple-touch-icon, theme-color, `robots.txt`, `sitemap.xml`) findes allerede i det ventende udkast — se `B30`/`B31`.** |
| `I17` | Socialt delebillede (`og:image`) til hjemmesiden | Hører til `I9`s runde og ikke før: begge er metadata på en side, der ikke er publiceret, og begge skal alligevel have den endelige adresse i hånden. **Leveret i det ventende udkast (13. august 2026) — se `B30`/`B31`.** |
...
| A48 | **Hvad skal være opfyldt, før "Beta" fjernes fra hjemmesidens header og forside?** | Fundet under gennemgangen af hjemmesidens ventende opdateringsudkast (13. august 2026). Et Beta-mærkat er ikke fundet i selve udkastet, hverken i header eller forside — spørgsmålet forudsætter enten en kommende tilføjelse eller et mærkat, denne gennemgang ikke fangede. Under alle omstændigheder mangler et eksplicit exit-kriterium: hvornår er sitet "færdigt nok" til at miste mærkatet? Kandidater: ejer-godkendelse af copy (`I8`s punkt 1 i [`features/hjemmeside-v1.md`](./features/hjemmeside-v1.md)), et vist antal dage uden en fundet fejl, eller en beslutning om aldrig at bruge mærkatet. | **Ejerens beslutning** — ingen ekstern udløser. |
| A49 | **"La Liga" (hjemmesiden) eller "Primera División" (`leagues.name` i databasen) — hvilket navn vinder?** | Hjemmesidens ventende turnerings-sektion (`site/index.html`) og `saadan-virker-det.html` skriver "La Liga", mens appens `leagues.name` for samme turnering er "Primera División". To veje: omdøb visningsnavnet i databasen til "La Liga" (det navn de fleste brugere kender turneringen som), eller ret sitet til "Primera División" (det officielle navn, appen allerede bruger konsekvent). Uanset hvad skal de to sige det samme — i dag gør de ikke. | **Ejerens beslutning**, eller næste gang `leagues`-navnet alligevel røres. |
| A50 | **Serveres `site/robots.txt` og `site/sitemap.xml` faktisk fra sitets ROD i det udrullede projekt?** | De to filer er nye i det ventende udkast og forudsætter, at Vercel-projektet for `leagly.app` serverer `site/`s indhold fra roden (`/robots.txt`, ikke `/site/robots.txt`) — en antagelse, der først kan efterprøves, når projektet er oprettet og sitet publiceret. | **Ejerens aflæsning, efter sitet er publiceret** (`I8`s resterende trin): åbn `https://leagly.app/robots.txt` og `https://leagly.app/sitemap.xml` direkte. |
...
| B30 | **Ret sælgesætningen fodbold-eksplicit og synk den i alle fem filer, før den ventende site-opdatering merges** | Et site-opdateringsudkast (til gennemgang 13. august 2026, endnu ikke merget) omformulerer forsidens `<meta name="description">` og hero-tekst fodbold-eksplicit — den nævner nu Superligaen, Premier League og de øvrige turneringer i stedet for den generiske "Gæt resultater mod dine venner…". `saelgesaetning.test.js` (`G97`) kræver, at `site/index.html`s description BEGYNDER med samme sætning som `index.html`s, og at heroen indeholder den — det gør udkastets nye ordlyd ikke. Merges udkastet uændret, går testen rødt i CI. Sætningen skal omformuleres (fodbold-eksplicit, som udkastet allerede peger imod) og synkes i alle fem filer, testen vogter: `index.html`, `src/screens/Auth.jsx`, `site/index.html`, `api/invite-preview.js`, `README.md`. Se `B31` for hvad udkastet leverer, når det er merget. | Lille–mellem — fem filer, men selve ordlyden er en produktbeslutning, ikke kun en synkronisering |
| B31 | **Ryd/skrump `I17` og `I9`s rækker i denne fil, når den ventende site-opdatering merges** | Udkastet (samme som `B30`) leverer reelt `I17` (og:image 1200×630, og:title/description, twitter:card) og dele af `I9` (canonical, favicon, apple-touch-icon, theme-color, `robots.txt`, `sitemap.xml`) — allerede skrevet i `site/index.html`s `<head>` og to nye filer i `site/`. Ingen af de to rækker kan lukkes eller skrumpes, før koden faktisk ligger i repoet. | Lille — opslag og redigering, når `B30` er rettet og udkastet merget |
| B32 | **Fjern Champions League's "Fra ligafasen"-forbehold på hjemmesidens turneringsliste** | `site/index.html`s ventende turnerings-sektion viser Champions League med mærkatet "Fra ligafasen", fordi ligafasen endnu ikke er lodtrukket (samme forudsætning som `B8`/`B28`). Mærkatet skal fjernes samtidig med, at `B28`s kickoff-aflæsning gentages for CL. | Lille — én linje, samme udløser som `B28` |
| B33 | **Ret hjemmesidens `.html`-endelser, hvis site-Vercel-projektet får clean URLs slået til** | Det ventende udkasts `<link rel="canonical">`, `sitemap.xml` og alle interne links (nav, footer, CTA'er) bruger fulde `.html`-endelser (`saadan-virker-det.html` osv.). Slås clean URLs til på sitets Vercel-projekt, skal alle tre rettes i samme ombæring — ellers peger canonical og sitemap på URL'er, indstillingen selv gør forældede. | Lille — men rammer flere filer på én gang |
...
| G103 | **Sitets story-eksempler bliver motorens ægte formuleringer — endnu en kobling, ingen vagt holder øje med.** | Det ventende site-opdateringsudkast erstatter sitets opdigtede story-citater ("Du har slået Anders fem runder i træk") med eksempler i Story Engine v3's faktiske format (⚔️/🔥/🧠-præfikser, "Du sluttede dagen 2 point fra X"). Det gør sitet mere ærligt, men skaber samtidig en ny kobling: ændres skabelonerne i `sql/story_engine_v3.sql`, følger sitets eksempler ikke automatisk med — nøjagtig den slags stille drift, `saelgesaetning.test.js` (`G97`) blev bygget for at forhindre for sælgesætningen. Ingen kode er rørt endnu — koblingen opstår først, når `B30`/`B31`s udkast rent faktisk merges. | Enten en vagt efter samme mønster som `saelgesaetning.test.js` (en test, der læser begge og sammenligner ordlyd/format) eller en note i [`features/hjemmeside-v1.md`](./features/hjemmeside-v1.md) om, at sitets story-eksempler er kopier og bør tjekkes manuelt, når `story_engine_v3.sql` ændres. | Lille (en test eller en note) |
...
| I22 | **Burger-menu eller færre navigationspunkter på hjemmesiden under 880px** | Sitets navigation (fem links + CTA-knap) wrapper over 2–3 rækker på mobil i det ventende opdateringsudkast (13. august 2026) — læsbart, men ikke stramt. To retninger, ingen valgt: en burger-menu (koster den ene JS-afhængighed, `I8` har bevidst fravalgt: "Ingen JS overhovedet") eller færre synlige punkter (fx fold "Om os" ind under et af de andre). | Ny — fundet under gennemgangen 13. august 2026 |
...
### 13. august 2026 (tiende kørsel) — indbakken tømt

**Listen er 33 → 42.** Ni linjer fra gennemgangen af hjemmesidens ventende
opdateringsudkast (13. august 2026, endnu ikke merget) blev til syv nye
rækker og tre nye åbne beslutninger; ingen lukket.

**`B30`** — udkastets forsidetekst er blevet fodbold-eksplicit, men
`site/index.html`s nye `<meta name="description">` begynder ikke længere med
sælgesætningen fra `index.html`, og heroen indeholder den heller ikke ordret.
`saelgesaetning.test.js` (`G97`) vil derfor gå rødt, hvis udkastet merges
uændret. Går til Tier 7 sammen med `I8`: ingen ekstern udløser, blot arbejde
der mangler at blive gjort, før merge.

**`B31`** — når `B30` er rettet og udkastet merget, leverer det reelt `I17`
(og:image, og:title/description, twitter:card) og dele af `I9` (canonical,
favicon, apple-touch-icon, theme-color, `robots.txt`, `sitemap.xml`). Går til
Tier 6: udløseren er selve mergen.

**`A48`** — hjemmesidens Beta-mærkat, som denne gennemgangs linje forudsætter,
er ikke fundet i selve udkastet; spørgsmålet om et exit-kriterium er skrevet
ned alligevel, med det forbehold tydeligt i konteksten.

**`A49`** — "La Liga" på sitets turneringsliste vs. `leagues.name` =
"Primera División" i appen; hvilket navn de to skal enes om, er ikke afgjort.

**`A50`** — om `site/robots.txt`/`sitemap.xml` faktisk serveres fra roden i
det udrullede projekt, kan først efterprøves, når sitet er publiceret. Går
til Tier 6, ikke Tier 1: adgangen findes ikke endnu.

**`B32`** — Champions League's "Fra ligafasen"-forbehold på hjemmesiden skal
fjernes samtidig med, at `B28`s kickoff-aflæsning gentages for CL. Samme
udløser, ny række, fordi handlingen (rette en hjemmeside-linje) er en anden
end `B28`s (gentage et opslag).

**`B33`** — sitets `.html`-endelser (canonical, sitemap, interne links) skal
rettes, hvis site-Vercel-projektet får clean URLs slået til. Går til Tier 6.

**`I22`** — sitets mobilnavigation wrapper over 2–3 rækker under 880px;
burger-menu eller færre punkter er ikke besluttet.

**`G103`** — udkastets story-eksempler er nu Story Engine v3's ægte
formuleringer (⚔️/🔥/🧠) i stedet for opdigtede citater, hvilket er en
kobling ingen vagt eller note holder i trit med `sql/story_engine_v3.sql`.
Koblingen opstår først, når `B30`/`B31`s udkast merges.

Niende kørsels log (`A46` og `A47` tilføjet, ingen afgjort) er ikke bevaret
her — begge står stadig i deres tabeller (Tier 1 og Åbne beslutninger), så
intet går tabt ved, at denne fil kun bærer den seneste kørsel.
