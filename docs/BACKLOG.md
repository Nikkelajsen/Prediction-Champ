...
| `I10` | Domænet peget på hjemmesiden OG appen | **Skrumpet 9. august 2026 (`B25`), afgjort 12. august 2026, repoets del skrevet 13. august 2026.** E-mail-halvdelen er lukket, formen er valgt (`leagly.app` → hjemmesiden, `app.leagly.app` → appen, ingen omdøbning af projektet), og **trin 6 + 7 ligger nu som kode: redirect i `vercel.json`, 23 CTA'er + README flyttet, to faldbacks rettet.** ✅ **Trin 1, 3 og 5 er kørt af ejeren 13. august 2026** — `app.leagly.app` svarer, Turnstile-værtsnavnet er udvidet, og spærringen på koden er dermed ophævet. **Tilbage: trin 4** (Supabase Site URL — bør køres først), **udrulningen af trin 6**, `B21`s GitHub-omdøbning og **trin 8** (beviserne, som er det, der gør "meldt" til "bestået"). Trin 2 er stadig gated af `I8`. Runbog i [`DOMAENE.md`](./DOMAENE.md). CSP'en skal IKKE røres (to origins deler ikke headere). |
