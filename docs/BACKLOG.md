...
| # | Hvad | Hvorfor her |
|---|---|---|
| `A37` | **Aflæs, om der allerede findes en liga uden en levende administrator** | Ét read-only opslag, der må køres direkte mod produktionen: `select * from rehearsal.leagues();` efter `sql/dev/anonymize_rehearsal.sql`. Kolonnen `levende_admins = 0` er en liga, ingen kan administrere igen — nogensinde. Opslaget afgør, om rækken er en fremtidig fælde eller en, der allerede er sprunget. |
