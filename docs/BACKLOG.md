...
**`A37`s aflæsning er kørt mod produktionen 7. august 2026 (nat), og den er den
mest lærerige af dagens tre.** Svaret var ja — der findes allerede en frossen
liga — men den er tom, og det er de GRØNNE rækker, der bærer oplysningen: fire
rigtige ligaer med 5–9 medlemmer og præcis én levende admin hver. **En
aflæsning, der bekræfter en frygt, kan altså stadig flytte den:** faren er ikke
den ene røde række, den er de fire grønne. Sidegevinst: aflæsningen satte et
spørgsmålstegn ved `G76`s egen præmis, fordi en lukket konto i produktion ikke
fandtes 5. august.
...
| # | Hvad | Hvorfor her |
|---|---|---|
| `A37` | ~~Aflæs, om der allerede findes en liga uden en levende administrator~~ **AFLÆST 7. august 2026 — svaret er ja.** Tilbage: beslutningen | Aflæsningen er lavet og står i beslutningstabellen. **Kort: én frossen liga (`Testerr`), og den er harmløs — dens eneste medlem ER den lukkede konto.** Men de fire rigtige ligaer har 5–9 medlemmer og præcis ÉN levende admin hver, så eksponeringen er 28 mennesker, der hver især er én opretter-lukning fra samme tilstand. Rækken er dermed ikke længere et opslag, kun et valg. |
...
| `G76` | **Luk en testkonto fra APPEN mod staging** — men tjek FØRST, om det allerede er sket | Skrumpet 7. august 2026: databasehalvdelen er prøvet af og var ren (se rækken i gældstabellen). Tilbage står det, SQL ikke kan nå — knappen i Profil, `api/delete-account.js` og soft-sletningen i `auth.users` — altså præcis den kæde, hvis halve tilstand (`kun_anonymiseret`) er dokumenteret og aldrig set. 🔎 **`A37`s aflæsning satte et spørgsmålstegn ved rækkens præmis:** der ER en lukket konto i produktion (`Testerr`s eneste medlem), og `I16` talte nul så sent som 5. august. Altså er anonymiseringen kørt i produktion siden da. **Om HELE kæden kørte, eller kun RPC'en, afgøres af ét felt:** `select deleted_at from auth.users where id = '<id>'` — er det sat, har `api/delete-account.js` kørt, og rækken er reelt lukket; er det null, er kontoen i præcis den halve tilstand, rækken frygter, og så er den ikke en test mere, men en oprydning. 🛑 **`sql/dev/anonymize_rehearsal.sql` hører til staging og INTET andet sted:** den installerer et skema og en funktion, der bliver stående. Produktionen aflæses med `sql/checks/league_admin_coverage.sql`. |
