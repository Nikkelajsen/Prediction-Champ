...
| Permanente ligaer | 🟢 Bygget (afventer DB-kørsel) | Liga-laget v1 leveret: `groups`/`group_members` + liga-side. Kør `sql/groups.sql` + verificér på staging, før det er live. Spec: [`features/liga-laget-v1.md`](./features/liga-laget-v1.md). |
| Konkurrencer inde i ligaer | 🟢 Bygget (afventer DB-kørsel) | `competitions.group_id` + liga-dropdown ved oprettelse + liga-side viser ligaens konkurrencer. Liga-løse konkurrencer bevares under "Øvrige". |
| Frivillig tilmelding pr. konkurrence | 🟢 Bygget (afventer DB-kørsel) | Deltag/Forlad pr. konkurrence på liga-siden. Framelding blokeres af RLS, hvis man har tips på låste kampe. |
...
| 3 | **Liga-laget (permanente fællesskaber)** | Bogens vigtigste strukturelle princip og den største ændring. En gruppe-tabel oven på de eksisterende konkurrencer (konkurrencer får et gruppetilhør, medlemskab flyttes til gruppen, til-/framelding pr. konkurrence). **✅ Bygget (juli 2026): [`features/liga-laget-v1.md`](./features/liga-laget-v1.md)** — alle 3 faser leveret i kode (build + 45 tests grønne). **Mangler kun: kør `sql/groups.sql` i Supabase + verificér på staging.** | Stor |
...
| Juli 2026 | Liga-laget v1 bygget (alle 3 faser: DB-fundament, liga-UI, blød migrering). Hjem-gruppering pr. liga udskudt som kosmetisk polish. | Faserne er tæt koblede, og `move_competition_to_group` hører naturligt til på liga-siden — leveret samlet. Går live, når `sql/groups.sql` er kørt og verificeret på staging. |
