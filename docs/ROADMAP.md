...
| 3 | **Liga-laget (permanente fællesskaber)** | Bogens vigtigste strukturelle princip og den største ændring. En gruppe-tabel oven på de eksisterende konkurrencer (konkurrencer får et gruppetilhør, medlemskab flyttes til gruppen, til-/framelding pr. konkurrence). Kan indføres uden at rive det eksisterende ned — Ligaer-fanen er allerede navngivet til det. **Spec godkendt (juli 2026): [`features/liga-laget-v1.md`](./features/liga-laget-v1.md)** — 3 faser (DB-fundament → liga-UI + turnerings-omdøbning → blød migrering), klar til implementering. | Stor |
