...
3. Opret ét nyt cron-job.org-job for turneringen, med `SYNC_SECRET` i `x-sync-secret`-headeren. *(Rettet 2. august 2026: her stod, at nye jobs ikke **bør** bruge `?secret=`-fallbacken. Den findes ikke længere — `A11` er lukket, og query-vejen giver 401, så et job uden headeren kører aldrig.)* **Tilføj derefter jobbet i [`docs/CRON.md`](../CRON.md)** — registeret dér er kilden til, hvilke jobs der findes, og det er kun sandt, hvis det vedligeholdes. *(Tilføjet 30. juli 2026: registeret fandtes ikke, da denne spec blev skrevet.)*
