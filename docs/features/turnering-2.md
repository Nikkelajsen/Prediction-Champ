...
- **`full_season` på tværs af turneringer** (juli 2026): multivalg af turneringer ~~+ stages pr. turnering~~ (`mode_params.tournaments`), materialiseret i `competition_matches`. `custom`/`random` var allerede turnerings-løse med filter.
  ...
  **Rettet igen efter levering (21. august 2026, `G8`):** efterfyldningens regel 3 ("en runde, der er gået i gang, vokser aldrig") regnede rundens start pr. TURNERING, fordi `backfillCompetitionMatches()` kaldes med én `season_id` ad gangen. En runde er en kalenderuge og hører ikke til én turnering, så for netop de flerturnerings-konkurrencer, dette punkt gjorde mulige, var reglen sand pr. turnering og falsk pr. konkurrence: den ene ligas kamp kunne have sat runden i gang, mens efterfyldningen lagde en ny kamp ind i den anden. Rundens start regnes nu over alle konkurrencens turneringer. Ingen eksisterende konkurrence ændrer adfærd — `mode_params.tournaments` har aldrig været skrevet i produktion, hvilket er hele `G8`s pointe.
...
**Rettet efter leveringen (14. august 2026, `A55`): Scotland Premiership ER officiel nu.** Sætningen ovenfor — *"Scotland Premiership er sat `is_official = false`"* — var sand fra 31. juli til 14. august 2026 og er det ikke længere; forfremmelsen står i `sql/tournament_scotland_promote.sql` (#64) og i `DECISIONS.md` samme dato. Generalprøven var hele begrundelsen for `false`, og den er overstået: turneringen har kørt to uger med rigtige kampe, rigtige tips og en live-sync, og maskineriet — `scope`-dimensionen, per-turnering-stillingerne, `scopeNote()` — er afprøvet af netop den. **To ting, spec'en forudsagde, skete af sig selv:** *"maskineriet står klar til den dag, en rigtig turnering #3 forfremmes"* gjaldt også for turnering #2, og *"linjen forsvinder af sig selv, den dag alle synlige turneringer er officielle"* er nu sket — `scopeNote()` returnerer `null`, når der ingen uofficielle er, så ingen kode skulle røres for at fjerne den. Afsnittene nedenfor står som skrevet, fordi rækkefølgen er pointen: `is_official` blev tegnet for en turnering, der ikke skulle vinde titler, og den kolonne bruges stadig af den næste generalprøve.
