...
| Story Engine | `story_viewed`, `story_shared` | `src/screens/hjem/DayCard.jsx`, `RoundStory.jsx`. `story_shared` bæres af tre flader, som skelnes på `metadata.from`: `frame:ROUND_SUM`/`frame:RATING` (rundestoryen), `milestone` (karriereprofilen) og — siden 15. august 2026 (`I22`) — `day_card`. **Opdelingen AFLÆSES siden `B37` (16. august 2026)** i Engagement-sektionens "Hvorfra deles der?" — indtil da blev alle tre talt som ét tal. Milepæls-delinger holdes samtidig ude af Story Engine-regeltabellens `Delt`-kolonne: nøglen står i `rule`, og `MONTH_CHAMP` er bevidst både en milepælsnøgle og en story-regel |
| Deling af en stilling | `standings_shared` | `src/screens/BoardScreen.jsx` (`shareStandings`). **Bevidst et nyt navn og ikke en `metadata.via` på `story_shared`:** en `via` skelner mellem KILDER til det samme trin, og det er derfor dagskortets deling ER `story_shared`. En stilling har hverken `stories`-række, regel eller nyhedsværdi og ville forurene enhver opgørelse over Story Engine, den blev talt med i — samme afvejning som `invite_landed`. Metadata: `rows` (feltets størrelse) og `complete`. 🔴 Navnet kræver [`#67 analytics_standings_share.sql`](../../sql/analytics_standings_share.sql) kørt FØR frontenden udrulles; ellers svælges hver insert stille, og tallet står tavst på nul |
