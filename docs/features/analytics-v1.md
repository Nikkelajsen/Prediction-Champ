...
| Story Engine | `story_viewed`, `story_shared` | `src/screens/hjem/DayCard.jsx`, `RoundStory.jsx`. `story_shared` bæres af tre flader, som skelnes på `metadata.from`: `frame:ROUND_SUM`/`frame:RATING` (rundestoryen), `milestone` (karriereprofilen) og — siden 15. august 2026 (`I22`) — `day_card` |
