...
### Opret konkurrence
Det eksisterende flow får ét nyt felt øverst: **"Liga"** (dropdown over ligaer, man er medlem af, ~~+ "Ingen liga"~~). *(Rettet efter levering, august 2026: **"Ingen liga" findes ikke længere.** Udkastets valgmulighed gjorde overgangstilstanden til et lovligt valg — og den var samtidig defaulten, indtil onboarding v1 flyttede den. En ny konkurrence skal høre til en liga; guarden står i `createCompetition`, så guiden og skærmen ikke kan divergere. Har man ingen liga, oprettes den **inde i opret-flowet** — navnefelt + "Opret liga" dér, hvor dropdownen ellers står — så man gerne må begynde med konkurrencen uden at miste det halvvalgte undervejs. Sidste punkt i dropdownen er "+ Opret ny liga…". Reglen kan ikke være et `not null` i databasen: `on delete set null` gør en konkurrence liga-løs, når ligaen slettes, og de gamle liga-løse rækker skal blive ved med at virke — se afsnit 6.)* Kommer man fra liga-siden, er ligaen forvalgt. *(Rullet tilbage ved A7, juli 2026: udkastet skjulte konkurrence-invite-linket for liga-konkurrencer. Begge link-typer er nu permanente, og **stedet afgør** — konkurrence-siden deler altid `?join=`, liga-siden altid `?liga=`. Et join via konkurrence-linket melder én ind i BEGGE, jf. A8.)* Opretteren tilmeldes selv automatisk (som i dag).
...
| Fil | Ændring | Omfang |
|---|---|---|
| `src/lib/data/groups.js` + `src/lib/data/competitions.js` *(oprindeligt `src/lib/data.js`; helpers flyttet ved opsplitningen, `G1`)* | Nye helpers — leveret som `loadMyGroups`, `loadGroupDetail`, `createGroup`, `loadGroupByCode`, `joinGroup`, `leaveGroup`, `deleteGroup`, `joinCompetition`, `leaveCompetition`, `moveCompetitionToGroup`. *(Udkastets `joinGroupByCode`/`joinCompetitionInGroup` blev til `loadGroupByCode` + `joinGroup` og et fælles `joinCompetition(token, userId, compId, groupId)`, så begge veje ind i en konkurrence bruger samme funktion — A7 viste, hvad der sker, når de ikke gør.)* **Rettet efter levering (10. august 2026, `A40`):** `joinGroup` og `loadGroupByCode` findes ikke længere — opslaget blev til `invite_lookup()`/`invite_preview()` og indmeldingen til `accept_invite()`, fordi en liga ikke længere må kunne læses eller tilmeldes på sit id alene. **Rettet efter levering (12. august 2026, `G95`):** `createGroup` skriver ikke længere selv de to rækker. Den kalder RPC'en `create_group(navn)` (`sql/create_group.sql`), som skriver ligaen OG opretterens admin-række i ét statement — spec'ens tostrengede klient-skrivning var netop det vindue, hvor en fejl kunne efterlade en liga uden medlemmer. Helperen har mistet sin `userId`-parameter af samme grund som `joinGroup` er væk (`A40`): hvem der handler, afgør databasen. | Mellem |
