...
| **F2** | ~~Én pointkilde~~ **✅ Besluttet (juli 2026): 3-1-0 fastfryses overalt.** `rules`-feltet er historisk; alle opgørelser er altid 3/1 (DB'ens `pc_points` er kilden). Karriereprofilens tal bygger direkte på `pc_points`/stillings-views'ene. | Bekræftet mod `sql/schema.sql`: `pc_points()` hardkoder 3/1 og ignorerer `rules` — beslutningen matcher den faktiske adfærd. Frontendens `rules`-læsning kan afvikles som separat oprydning — følges som `G3` i [`../BACKLOG.md`](../BACKLOG.md) (31. juli 2026), så oprydningen ikke kun står i en leveret spec. |
...
| K2 | **Per-turnering-opdeling fra start?** `ratings.scope` er forberedt til per-liga-rating. **Fortsat åben** — indekseret i [`../BACKLOG.md`](../BACKLOG.md) (31. juli 2026), så den kan ses uden at åbne denne spec; sandheden står her. | Nej — vent til turnering #2 er i drift ([`turnering-2.md`](./turnering-2.md), `B2`), ellers bygges en vælger uden indhold. |
