...
*(Rettet efter levering, `G116`, 14. august 2026: **gen-forsøget virkede ikke.** Budgettet var præcis 2 × tidsgrænsen, og gen-forsøget krævede en hel tidsgrænse tilbage — mens et timeout pr. definition har brugt hele grænsen. Det fyrede derfor aldrig i produktion, hvilket kunne aflæses på kørslernes varighed hos cron-job.org: 21,7 sekunder mod de ~41, to forsøg koster. Det blev derefter fjernet igen af `G117` samme dag: cron-job.org afbryder kaldet efter **30 sekunder**, som er maksimum på planen, og to kald à 20 s er ~42. Der er plads til **ét** udgående kald pr. kørsel, budgettet er sat til 25 s, og gen-forsøget er jobbet selv — det kører jo hvert minut. Kæden af grænser står i [`../CRON.md`](../CRON.md).)*

*(Rettet efter levering, `G113`, 15. august 2026: **tallene bor ikke længere i `sportmonks.js`.** `LIVE_TIMEOUT_MS`, `LIVE_BUDGET_MS`, `LIVE_MIN_CALL_MS` og regnskabet `createLiveBudget()` er flyttet til `api/_shared.js`, og **football-data.org bruger nu de samme**. Grænserne er udledt af KALDEREN — cron-job.orgs 30 sekunder — og ikke af leverandøren, så placeringen hos den ene var en tilfældighed: det var Sportmonks, der var langsom den aften. Undervejs kom et fund, backloggens række ikke nævnte: football-datas 429-pause kunne vare op til **60 sekunder**, altså dobbelt så længe som hele det vindue, kørslen har. Pausen er nu betinget af budgettet — er der plads til at vente OG nå et kald bagefter, ventes der; er der ikke, leveres 429'eren videre som en højlydt fejl. Gen-forsøget er gjort BETINGET, ikke fjernet. De to sæson-opslag er urørte: `sync-matches` kører hver 12. time og har tid.)*

*(Tilføjet efter levering, `G122`, 15. august 2026: **trin 1 har fået et tredje opslag, som kun kører ét minut i timen.** Vinduet `[nu − 6 t; nu + 15 min]` er også en GRÆNSE: en kamp, der er mere end 6 timer gammel og stadig står uden endeligt resultat, er usynlig for jobbet og ventede derfor på næste `sync-matches`-kørsel — op til 12 timer, hvor rating og stillinger står forkerte. I minut **41** spørges der derfor også på kampe i `[nu − 36 t; nu − 6 t]` uden resultat, med et loft på 40 kampe. De to vinduer støder op til hinanden uden overlap, så en kamp høres af præcis ét af dem. **Gatet på ét minut i timen er ikke pynt:** trin 2 er hele jobbets forbrugsbegrænsning, og en udsat kamp — hvis `kickoff_at` endnu ikke er skrevet om — ville uden gatet udløse et leverandørkald hvert minut i døgnet rundt. Prisen er 24 ekstra kald i døgnet i værste fald frem for 1.440, og latensen falder fra 12 timer til 1. Kvitteringen er `staleChecked` i kørslens resumé, som **mangler** uden for fejeminuttet og står `0` ved en fejning uden fund; `staleRescued` er kun til stede ved fund og er et spor efter et tabt slutfløjt, ikke en succes. Begrundelsen for hvert af de tre værn står i `api/sync-live.js` og i [`../DECISIONS.md`](../DECISIONS.md).)*
