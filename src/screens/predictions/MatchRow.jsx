...
function MatchRow({
  ...
  const status = played && showFinal ? <FinalBadge />
    ...
    ) : <span style={{ color: C.muted, fontSize: 12, whiteSpace: "nowrap" }}>{played ? "" : hhmm(m.kickoff_at, m.kickoff_tbd, m.kickoff_uncertain) || "–"}</span>;
      ...
      {locked ? (
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, minHeight: 40, padding: "7px 0" }}>
          <div style={{ flex: 1, minWidth: 0, display: "flex", alignItems: "baseline", gap: 8 }}>
            <span style={{ color: C.muted, fontSize: 12, whiteSpace: "nowrap" }}>{hhmm(m.kickoff_at, m.kickoff_tbd, m.kickoff_uncertain) || "–"}</span>
