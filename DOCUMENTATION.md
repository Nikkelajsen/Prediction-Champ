...
**Invitationer — to links, begge permanente (A7, juli 2026).** `?liga=<kode>` inviterer til fællesskabet; `?join=<kode>` inviterer direkte til én konkurrence. **Stedet afgør:** Invitér-knappen på liga-siden (`GroupScreen`) deler altid liga-linket, knappen på en konkurrences side (`BoardScreen`) altid konkurrence-linket. `BoardScreen` erstattede tidligere stiltiende konkurrence-linket med liga-linket for liga-konkurrencer, så man ikke kunne invitere til én bestemt konkurrence — substitutionen er fjernet, og med den `groups`-opslaget i skærmen. **A8 håndhæves i klienten, ikke i RLS:** INSERT-policyen `"join competition"` kræver kun `user_id = auth.uid()`, så det er `MainApp.confirmJoin` (deep-link) og `LigaerTab.joinByCode` (indsat kode), der melder joineren ind i konkurrencens liga, når `comp.group_id` er sat — liga-medlemskabet først, så en fejl ikke efterlader en halv tilstand. Bekræftelses-modalen nævner ligaen ved navn. Vil man senere håndhæve A8 i databasen, er stedet en INSERT-policy på `competition_participants` med `is_group_member(...)`.
