...
Det oprindelige skema-/rating-SQL ligger ikke i repoet (kun i Supabase). Indsæt det i `sql/`-mappen som `sql/schema.sql`, så hele databasen kan genskabes fra git — nødvendigt bl.a. for at opsætte staging, og en forudsætning for karriereprofilen og turnering #2 (se `docs/features/`). **Udførlig trin-for-trin-guide (forudsætter ingen forhåndsviden): [`sql/README.md`](./sql/README.md)** — inkl. en "lad Claude gøre det"-vej og en gør-det-selv-vej. Når filen er committet, kan de gentagne "skemaet er ikke i repoet"-forbehold (afsnit 5, 9, 17) fjernes, og ny SQL kan skrives/verificeres mod den faktiske DDL i stedet for det dokumenterede skema.
