...
Juli 2026 — Rettelse: konkurrence-link melder også ind i ligaen (deep-link)
Et `?join=<konkurrence-kode>`-deep-link for en konkurrence i en liga meldte kun brugeren ind i konkurrencen, ikke ligaen (kun den manuelle "Join med kode" gjorde begge dele). `confirmJoin()` i `MainApp.jsx` melder nu også ind i ligaen, når konkurrencen har `group_id` — så deep-link og manuel kode opfører sig ens (spec afsnit 6). Bekræftelses-modalen retter samtidig terminologien ("konkurrencen …", ikke "ligaen …") og nævner, at man samtidig bliver liga-medlem. Historiske deltagere (joinet før rettelsen) fanges af en idempotent backfill i `sql/groups.sql` (afsnit 7), der gør alle deltagere i liga-bundne konkurrencer til liga-medlemmer. **Kør `sql/groups.sql` igen i Supabase for at anvende backfillen.**
