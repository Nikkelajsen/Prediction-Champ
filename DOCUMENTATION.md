...
Opsætning: ÉT cron-job på cron-job.org, hvert minut, mod `https://<app>/api/sync-live` med headeren `x-sync-secret`. Ingen query-parametre — funktionen finder selv de relevante kampe på tværs af alle ligaer. `sync-matches`-jobbene (ét pr. liga, hvert 10.-15. minut) bevares uændret: de holder kampprogrammet, flyttede kampe og nye hold ajour.
