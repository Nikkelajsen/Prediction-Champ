...
Hjem — deadline-kort (næste rundes deadline + antal kampe der mangler tips + "Tip nu"-knap; helper `computeHomeTips`), rating-snapshot (rating, bevægelse, placering, formkurve → linker til Rating), og "Dine placeringer" (månedsliga øverst, dernæst konkurrence-placeringer grupperet pr. liga (liga-laget, afsnit 18) — liga-løse under "Øvrige"; har man ingen ligaer, vises de fladt; hver linker videre). Deadline-kortet er farvekodet: rødt når man mangler tips til næste runde, grønt ("Alt ok — alle tips er inde") når alt er tippet. Tom-tilstanden er liga-bevidst (`HjemTab` henter `loadMyGroups`): har man ingen aktive konkurrencer MEN er medlem af en liga, vises "Du er med i ligaen X, men har ikke tilmeldt dig en konkurrence endnu → Åbn ligaen og deltag" (ikke den forkerte "Du er ikke med i nogen ligaer endnu", som kun vises, når man hverken har konkurrencer eller ligaer). Derunder en live-oversigt over indeværende runde (resultat/tip/point pr. kamp + samlede point), der genindlæses hvert minut mens kampene spilles (helper `computeCurrentRound`). Kampe i gang vises med den nuværende stilling + LIVE-mærke (afsnit 8) — også på det foldede kort — mens færdigspillede kampe vises med resultat + point-pille. Nederst et opt-in-kort til push-notifikationer (afsnit 16), som kan skjules og forsvinder når man er tilmeldt.
...
Datakilde: ingen nye tabeller. Ét RPC `career_profile(profile_user_id uuid)` (`security definer`, `set search_path = public`, mønster som `admin_user_stats()`) returnerer al profil-data som `jsonb`: hoved (`profiles`, `ratings`, seneste `rating_history.delta`), titler (månedstitler fra `monthly_standings` + rundesejre fra `round_standings`, kun for **afsluttede** måneder/runder = alle kampe spillet), ratingkurve (`rating_history`), basistal og rivaler.

Adgang (K1): RPC'et er gated på relationen i stedet for admin — tilladt hvis `profile_user_id = auth.uid()` (egen profil), eller kalderen deler en liga (`group_members`) eller en konkurrence (`competition_participants`) med profilbrugeren; ellers `raise exception 'forbidden'` (skærmen viser en pæn fejltekst). Andres profil viser hoved, titler, kurve og basistal.

Privathed: milepæle og rivaler er personlige og vises kun på egen profil. Milepæle læses **separat** client-side via den eksisterende RLS på `stories` (kun egne rækker — `loadCareerMilestones` springer kaldet over for andres profil); rivaler (ren story-optælling af `payload->>'rival'` for regel `H2H_PASS`/`STREAK`, K3) kommer kun med i RPC-svaret, når `profile_user_id = auth.uid()`.
...
Frontend: `loadCareerProfile`/`loadCareerMilestones` (`data.js`, enhedstestet i `data.test.js`). Ratingkurven tegnes som en letvægts inline-SVG (`Sparkline` i `ProfileScreen.jsx`, ingen chart-bibliotek); de første <5 runder (provisorisk K-faktor) tegnes dæmpet/stiplet. Nås via rating-snapshot-kortet på Hjem (`openProfile(userId)`) og navne-klik i Rating-ranglisten (`RatingTab`); `MainApp` router `screen.type === "profile"`. Tom tilstand for nye brugere: hoved + venlig "din karriere er lige begyndt"-tekst, ingen nul-tabeller.

Engangsopsætning: kør `sql/career_profile.sql` i Supabase ("Run without RLS"). Idempotent. SQL'en er verificeret mod `sql/schema.sql` (afsnit 12).
---
